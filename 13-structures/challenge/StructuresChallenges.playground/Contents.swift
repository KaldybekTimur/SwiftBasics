...
/*
 ...
 • Методы - это поведение, определяющее функциональность типа.
 • Метод может получить доступ к данным экземпляра с помощью ключевого слова self.
 • Инициализаторы создают новые экземпляры типа. Они очень похожи на методы, которые называются init без возвращаемого значения.
 • Метод типа добавляет поведение к типу вместо экземпляров этого типа. Чтобы определить метод типа, вы добавляете перед ним модификатор static.
 • Вы можете открыть существующую структуру и добавить к ней методы, инициализаторы и вычисляемые свойства с помощью расширения.
 • Добавляя собственные инициализаторы в расширения, вы можете сохранить поэлементный инициализатор компилятора для структуры.
 • Методы могут существовать во всех названных типах - структурах, классах и перечислениях.
 */
